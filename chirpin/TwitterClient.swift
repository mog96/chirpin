...
class TwitterClient: BDBOAuth1RequestOperationManager {
    ...
    // Returns non-nil if an instance can be created.
    ...
    func homeTimelineWithParams(params: NSDictionary?, completion: (tweets: [Tweet]?, error: NSError?) -> ()) {
        ...
        // HOME TIMELINE
        TwitterClient.sharedInstance.GET("1.1/statuses/home_timeline.json", parameters: params, success: { (operation: AFHTTPRequestOperation!,
    ...
    func mentionsWithParams(params: NSDictionary?, completion: (tweets: [Tweet]?, error: NSError?) -> ()) {
        ...
        // HOME TIMELINE
        TwitterClient.sharedInstance.GET("1.1/statuses/mentions_timeline.json", parameters: params, success: { (operation: AFHTTPRequestOperation!,
    ...
    func composeTweet(params: NSDictionary?, completion: (tweet: Tweet?, error: NSError?) -> ()) {
        TwitterClient.sharedInstance.POST("1.1/statuses/update.json", parameters: params, success: { (operation: AFHTTPRequestOperation!,
    ...
    func loginWithCompletion(completion: (user: User?, error: NSError?) -> ()) {
        ...
        // Gets request from server. If user has recently logged in,
        // runs success code. Else, runs error code.
        TwitterClient.sharedInstance.fetchRequestTokenWithPath("oauth/request_token", method: "GET", callbackURL: NSURL(string: "chirpin://oauth"), scope: nil,
    ...
    func retweet(tweetID: Int?, completion: (status: Tweet?, error: NSError?) -> ()) {
        ...
        self.POST(retweetUrlString, parameters: nil, success: { (operation: AFHTTPRequestOperation!,
            
        response: AnyObject!) -> Void in
            var tweet = Tweet(dictionary: response as! NSDictionary)
            
            
            println("COMPLETE. WOOOOOOOP")
